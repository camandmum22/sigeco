...
public class ConvocatoriaFacade extends AbstractFacade<Convocatoria>
{
     ...
     *
     * */
    public Convocatoria getUltimaConvocatoria(){
        Convocatoria retorno = (Convocatoria)em.createQuery("SELECT fas.conCodigo FROM Fase fas WHERE fas.fechaFin = (select MAX(f1.fechaFin) FROM Fase f1)").getSingleResult();
        return retorno;
    }
